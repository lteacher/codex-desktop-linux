...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxWrapAppId(){let id=process.env.CODEX_LINUX_APP_ID||process.env.CODEX_APP_ID||\`codex-desktop\`;return /^[A-Za-z0-9._-]+$/.test(id)?id:\`codex-desktop\`}`,
    `function codexLinuxWrapMarkerPath(){let h=codexLinuxWrapHome();let d=process.env.XDG_STATE_HOME||(h&&${pathVar}.join(h,\`.local\`,\`state\`));return d?${pathVar}.join(d,codexLinuxWrapAppId(),\`wrapper-update-pending\`):null}`,
